...
//    Function SortCompare(iLo, iHi: TBFProgram): Integer;
//    Begin
//        Result := IfThen(iLo.Score > iHi.Score, 0, 1);
//    End;

Var
    I, J: Integer;
Begin
    //    ProgramPool.Sort(@SortCompare);

    For I := 0 To ProgramPool.Count - 2 Do
    Begin
        For J := I + 1 To ProgramPool.Count - 1 Do
        Begin
            If TBFProgram(ProgramPool[i]).Score <
                TBFProgram(ProgramPool[j]).Score Then
            Begin
                ProgramPool.Exchange(I, J);
            End;
        End;
    End;
End; }
...
Begin
    ...
    Begin
        If Random / FOwner.MutationRate < 1 Then
        Begin
            K := Trunc(20 * Random * Random);
            For J := 0 To K Do
                Insert(' ', Src, I);
            Inc(I, K);
        End;
    ...
    If Src <> '' Then
    Begin
        CS := FOwner.FEvClass.CommandSet + ' ';
        For I := 0 To Trunc(Length(Src) * Random * FOwner.MutationRate) Do
        Begin
            Src[1 + Random(Length(Src))] := CS[1 + Random(Length(CS))];
        End;
    End;

    For I := 1 To Trunc(Length(Src) * Random * FOwner.MutationRate) Do
    Begin
        Delete(Src, 1 + Random(Length(Src)), Trunc(10 * Random * Random));
    End;

    //Set the mutated source to be the current one ...
    Source := Src;
End;
